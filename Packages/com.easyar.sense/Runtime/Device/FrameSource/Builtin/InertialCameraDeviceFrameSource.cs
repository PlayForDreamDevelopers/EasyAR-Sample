...
{
    /// <summary>
    /// <para xml:lang="en"><see cref="MonoBehaviour"/> which controls <see cref="InertialCameraDevice"/> in the scene, providing a few extensions in the Unity environment.</para>
    /// <para xml:lang="en">This frame source is one type of 5DOF (rotation + horizontal translation) tracking device, and will output motion data in a <see cref="ARSession"/>.</para>
    /// <para xml:lang="zh">在场景中控制<see cref="InertialCameraDevice"/>的<see cref="MonoBehaviour"/>，在Unity环境下提供功能扩展。</para>
    /// <para xml:lang="zh">这个frame source是一种5DOF（旋转+水平位移）跟踪设备，在<see cref="ARSession"/>中会输出运动数据。</para>
    /// </summary>
    public class InertialCameraDeviceFrameSource : FrameSource, FrameSource.ISenseBuiltinFrameSource, FrameSource.IMotionTrackingDevice
    {
        /// <summary>
        ...
        private InertialCameraDevice device;
        ...
        internal protected override Optional<bool> IsAvailable => InertialCameraDevice.isAvailable();
        ...
        public void Open()
        {
            ...
            CameraDevice.requestPermissions(EasyARController.Scheduler, (Action<PermissionStatus, string>)((status, msg) =>
            {
                ...
                Close();
                device = new InertialCameraDevice();
                ...
                if (!openResult)
                {
                    device.Dispose();
                    device = null;
                    var error = $"{nameof(InertialCameraDevice)}.{nameof(InertialCameraDevice.open)} fail";
        ...
        /// <summary>
        /// <para xml:lang="en">Resets states.</para>
        /// <para xml:lang="zh">重置状态。</para>
        /// </summary>
        public void ResetInertial() => device?.reset();
