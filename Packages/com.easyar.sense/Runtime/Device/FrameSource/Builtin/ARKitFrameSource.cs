...
{
    /// <summary>
    /// <para xml:lang="en"><see cref="MonoBehaviour"/> which controls ARKit camera device (<see cref="ARKitCameraDevice"/>) in the scene, providing a few extensions in the Unity environment.</para>
    ...
    /// <para xml:lang="zh">在场景中控制ARKit相机设备（<see cref="ARKitCameraDevice"/>）的<see cref="MonoBehaviour"/>，在Unity环境下提供功能扩展。</para>
    /// <para xml:lang="zh">这个frame source是一种运动跟踪设备，在<see cref="ARSession"/>中会输出运动数据。</para>
    /// </summary>
    public class ARKitFrameSource : FrameSource, FrameSource.ISenseBuiltinFrameSource, FrameSource.IMotionTrackingDevice
    {
        /// <summary>
        ...
        private ARKitCameraDevice device;
        [HideInInspector, SerializeField]
        private bool hasDesiredSize;
        [HideInInspector, SerializeField]
        private Vector2Int desiredSize = new(1920, 1440);
        [HideInInspector, SerializeField]
        ...
        /// <summary>
        /// <para xml:lang="en">Supported frame rates of current device. Only usable when <see cref="Opened"/> is true.</para>
        /// <para xml:lang="zh">当前设备支持的所有帧率。仅在<see cref="Opened"/>为true时可用。</para>
        /// </summary>
        public List<double> SupportedFrameRate
        {
            get
            {
                List<double> list = new();
                for (var i = 0; i < device.supportedFrameRateCount(); ++i)
                {
                    list.Add(device.supportedFrameRate(i));
                }
                return list;
            }
        }
        ...
        /// <summary>
        /// <para xml:lang="en">Current frame rate. Only usable when <see cref="Opened"/> is true.</para>
        /// <para xml:lang="en">The nearest value from <see cref="SupportedFrameRate"/> will be used when set.</para>
        /// <para xml:lang="zh">设备的当前帧率。仅在<see cref="Opened"/>为true时可用。</para>
        /// <para xml:lang="zh">set会使用<see cref="SupportedFrameRate"/>中数值最接近的大小。</para>
        /// </summary>
        ...
        /// <summary>
        /// <para xml:lang="en">Desired camera frame rate. Only effective if modified before the <see cref="DeviceOpened"/> event.</para>
        /// <para xml:lang="en">The nearest value from <see cref="SupportedFrameRate"/> will be used. Default value will be used if not set.</para>
        /// <para xml:lang="zh">期望的相机图像帧率，在<see cref="DeviceOpened"/>事件前修改才有效。</para>
        /// <para xml:lang="zh">会使用<see cref="SupportedFrameRate"/>中数值最接近的大小。未设置将使用默认值。</para>
        /// </summary>
        public Optional<double> DesiredFrameRate
        {
            get => hasDesiredFrameRate ? desiredFrameRate : Optional<double>.Empty;
            set
            {
        ...
        internal protected override Optional<bool> IsAvailable => ARKitCameraDevice.isAvailable();
        internal protected override bool CameraFrameStarted => started;
        internal protected override List<FrameSourceCamera> DeviceCameras => Opened ? new List<FrameSourceCamera> { new(device.type(), device.cameraOrientation(), Size, new((float)FrameRate, (float)FrameRate)) } : new();
        ...
        public void Open()
        {
            ...
            CameraDevice.requestPermissions(EasyARController.Scheduler, (Action<PermissionStatus, string>)((status, msg) =>
            {
                ...
                {
                    device.setFrameRate(DesiredFrameRate.Value);
                }
        ...
        internal override string DumpLite()
        {
            ...
            data += $"Source: {ARSessionFactory.DefaultName(GetType())} ({enabled}), {(Opened ? Size : "-")}, {(Opened ? FrameRate : "-")}" + Environment.NewLine;
            return data;
        }
