...
{
    /// <summary>
    /// <para xml:lang="en"><see cref="MonoBehaviour"/> which controls camera image rendering in the scene. Only effective when <see cref="FrameSource.IsCameraUnderControl"/> of session frame source is true.</para>
    /// <para xml:lang="zh">在场景中控制camera图像渲染的<see cref="MonoBehaviour"/>。仅当session的frame source<see cref="FrameSource.IsCameraUnderControl"/>为true时有效。</para>
    /// </summary>
    ...
    public class CameraImageRenderer : MonoBehaviour
    {
        ...
        private ImageMaterial arMaterial;
        ...
        /// <summary>
        /// <para xml:lang="en">Enable/disable camera image rendering (if controlled by EasyAR) when <see cref="ARSession"/> is running.</para>
        /// <para xml:lang="zh"><see cref="ARSession"/>运行时启用/禁用相机图像渲染（如果由EasyAR控制）。</para>
        /// </summary>
        ...
        {
            if (targetCamera && targetCamera != session.Assembly.Camera)
            {
                OnSessionStop(false);
            }
            arSession = session;
        ...
        internal void OnSessionStop(bool keepLastFrame)
        {
            if (!keepLastFrame)
            {
                ClearCamera();
        ...
        internal void UpdateCamera(Optional<Tuple<OutputFrame, Quaternion>> frame)
        {
            if (frame.OnSome)
            {
                OnFrameChange(frame.Value.Item1);
            }
            OnFrameUpdate();
        }

        internal void ClearCamera()
        {
            arMaterial?.Dispose();
            arMaterial = null;
            material = null;
        ...
        private void OnFrameChange(OutputFrame outputFrame)
        {
            ...
            using (var frame = outputFrame.inputFrame())
            {
                ...
                {
                    if (arMaterial?.CanReplace(image) ?? false)
                    {
                        arMaterial.UncheckedReplace(image);
                    }
                    else
                    {
                        arMaterial?.Dispose();
                        arMaterial = new ImageMaterial(image);

                        material = arMaterial.Material;
                        UpdateCommandBuffer(targetCamera, material, invertCulling);
                        request?.UpdateCommandBuffer(targetCamera, material, invertCulling);
                    }
                }
                ...
                {
                    Debug.LogError($"Unable to render image, missing {nameof(cameraParameters)}");
                }
            }
        }
        ...
        {
            if (!enabled && request == null && OnFrameRenderUpdate == null || cameraParameters == null)
            {
                return;
            }
        ...
        private void UpdateCommandBuffer(Camera cam, Material material, bool invertCulling)
        {
            ...
            if (enabled)
            {
                if (SystemUtil.RenderPipeline != SystemUtil.RenderPipelineType.Builtin) { return; }
