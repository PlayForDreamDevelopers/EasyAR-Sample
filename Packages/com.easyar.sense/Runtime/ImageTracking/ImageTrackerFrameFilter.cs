...
{
    /// <summary>
    /// <para xml:lang="en"><see cref="MonoBehaviour"/> which controls <see cref="ImageTracker"/> in the scene, providing a few extensions in the Unity environment.</para>
    /// <para xml:lang="zh">在场景中控制<see cref="ImageTracker"/>的<see cref="MonoBehaviour"/>，在Unity环境下提供功能扩展。</para>
    /// </summary>
    public class ImageTrackerFrameFilter : FrameFilter, FrameFilter.IFeedbackFrameSink, FrameFilter.IOutputFrameSource, FrameFilter.ICenterTransform, FrameFilter.IHFlipHandler
    {
        /// <summary>
        /// <para xml:lang="en">Tracking mode. Only effective if modified before the session starts.</para>
        /// <para xml:lang="zh">跟踪模式，在session启动前修改才有效。</para>
        /// </summary>
        ...
        private Dictionary<int, ImageTargetController> allTargetController = new();
        private bool horizontalFlip;
        private TargetCenterTransformHelper targetCenterTransformHelper = new();
        private List<ImageTargetController> holdingTargets = new();
        private Dictionary<ImageTargetController, Coroutine> loadingTargets = new();
        ...
        /// <summary>
        /// <para xml:lang="en"><see cref="ImageTargetController"/> that has been loaded.</para>
        /// <para xml:lang="zh">已加载的<see cref="ImageTargetController"/>。</para>
        /// </summary>
        public List<ImageTargetController> Targets => allTargetController.Select(c => c.Value).ToList();
        ...
        internal override bool IsAvailable => ImageTracker.isAvailable();
        ...
        internal override void OnSessionStart(ARSession session)
        {
            ...
            using (var config = new ImageTrackerConfig())
            {
                config.setTrackerMode(TrackerMode);
                tracker = ImageTracker.createWithConfig(config);
            }
            tracker.setSimultaneousNum(simultaneousNum);
        ...
        internal void Hold(ImageTargetController controller)
        {
        ...
        internal void Unhold(ImageTargetController controller)
        {
            if (!holdingTargets.Contains(controller)) { return; }
        ...
        private void LoadTarget(ImageTargetController controller)
        {
        ...
        private IEnumerator LoadDataAndLoadTarget(ImageTargetController controller)
        {
            ...
            if (controller.LoadingResult.OnNone)
            {
                ...
                yield return ImageTargetDataLoader.Load(controller.LoadingSource.Value, worker, (target) =>
                {
                    if (!controller) { return; }
        ...
        private void UnloadTarget(ImageTargetController controller)
        {
        ...
        private ImageTargetController TryGetTargetController(int runtimeID)
        {
