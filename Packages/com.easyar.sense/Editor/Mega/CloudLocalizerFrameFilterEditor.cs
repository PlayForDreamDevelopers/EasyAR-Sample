...
namespace easyar
{
    [CustomEditor(typeof(CloudLocalizerFrameFilter), true)]
    class CloudLocalizerFrameFilterEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.HelpBox($"Package com.easyar.mega is required to use {nameof(CloudLocalizerFrameFilter)}", MessageType.Error);
#else
            DrawDefaultInspector();

            var filter = (CloudLocalizerFrameFilter)target;
            ...
            using (_ = new GUILayout.VerticalScope(GUI.skin.box))
            {
                ...
                if (GUILayout.Button("Trigger Resolve", GUILayout.Height(30)))
                {
                    filter.Resolve((response) =>
                    {
                        if (!this) { return; }

                        var str = "[" + DateTime.Now.ToString("HH:mm:ss") + "] Resolve: " + response.Status;
                        ...
                        logs.Enqueue(Tuple.Create(DateTime.Now, str, response.Status == MegaTrackerLocalizationStatus.Found || response.Status == MegaTrackerLocalizationStatus.NotFound || response.Status == MegaTrackerLocalizationStatus.MissingSpotVersionId));
                        Repaint();
                    });
                }
